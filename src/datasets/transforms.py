...
except Exception:  # pragma: no cover - fallback untuk environment tanpa TF
    tf = None
...
if tf is not None:
    ...
    class SmallGaussianNoise:  # pragma: no cover
        pass


    class SmallRandomErasing:  # pragma: no cover
        pass


    def build_training_augmentation():  # pragma: no cover
        raise ImportError("TensorFlow tidak tersedia. build_training_augmentation() membutuhkan TensorFlow")
...
def validate_train_split(split_dir: Path) -> Dict[str, int]:
...
def print_augmentation_summary(split_dir: Path) -> None:
    class_distribution = validate_train_split(split_dir)
    total_train = sum(class_distribution.values())

    print("\n=== Augmentasi Training On-the-Fly ===")
    print("Sumber train split :", (split_dir / "train").resolve())
    ...
    print("-" * 60)
    print("Catatan: augmentasi tidak membuat file baru, hanya diterapkan saat training.")
